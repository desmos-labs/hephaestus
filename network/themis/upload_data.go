...
func (c *Client) UploadData(username string, provider string, data *sign.SignatureData) error {
	...
	req, err := http.NewRequest("POST", fmt.Sprintf("%s/%s/data", c.host, provider), bytes.NewBuffer(bodyBz))
	if err != nil {
		return err
	}
